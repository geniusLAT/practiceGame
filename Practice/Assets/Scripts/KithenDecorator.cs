...
public class KithenDecorator : Decorator
{
    ...
    public override void Decorate(Architector.Room room)
    {
        ...
        while (potentials.ToArray().Length > 0)
        {
            Debug.Log("d=" + d);
            Architector.Cell place = potentials[Random.Range(0, potentials.ToArray().Length)];
            potentials.Remove(place);
            room.architector.PutFurniture(decors[d], place.a, place.b);
            if (d < 2)
            {
                d++;
            }
        ...
        for (int i = 0; i < Random.Range(0, 2); i++)
        {
            ...
            if (potentials.ToArray().Length > 0)
            {
                ...
                GameObject table = room.architector.PutFurniture(decors[5], place.a, place.b);
